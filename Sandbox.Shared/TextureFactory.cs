...
public static class TextureFactory
{
    ...
    public static Texture2D CreateFilledCircleTexture(GraphicsDevice device, Color centerColor, Color borderColor,
        int radius, int size)
    {
        var texture = new Texture2D(device, size, size);
        var colorData = new Color[size * size];
        ...
        for (var x = 0; x < size; x++)
        {
            for (var y = 0; y < size; y++)
            {
                ...
                var posDistance = pos.LengthSquared();
                colorData[index] = posDistance <= radius * radius
                    ? (Color.Lerp(centerColor, borderColor, posDistance / (radius * radius)))
                    : Color.Transparent;
            }
        }
